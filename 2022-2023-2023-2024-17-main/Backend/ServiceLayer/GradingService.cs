...
namespace IntroSE.Kanban.Backend.ServiceLayer
{
    ...
    public class GradingService
    {
        ...
        private StartSession head;
        public GradingService()
        {
            head = new StartSession();
            boardService = new BoardService(head); 
            userService = new UserService(head); 
            taskService = new TaskService(head);
        ...
        {
            return taskService.AdvanceTask(email, boardName, columnOrdinal, taskId);
        }
        /// <summary>
        ...
        {
            string columns1 = boardService.GetColumn(email, boardName, columnOrdinal);
            ...
            else
            {
                ...
                LinkedList<TaskToSend> taskList = new LinkedList<TaskToSend>();
                int index = 0;
                TaskToSend[] tasks = new TaskToSend[ans1.Count];
                foreach (Task task in ans1)
                {
                    TaskToSend temp = new TaskToSend(task);
                    taskList.AddLast(temp);
                    tasks[index] = temp;
                    index = index + 1;
                }
                
                Response r = new Response(tasks, null);
        ...
        {
            string columns1 = boardService.InProgressTasks(email);
            Response response1 = JsonSerializer.Deserialize<Response>(columns1);
            if (response1.ErrorMessage != null)
                return columns1;
            else
            {
                ...
                List<TaskToSend> taskToSends = new List<TaskToSend>();
                foreach (Task task in ans1)
                {
                    TaskToSend temp = new TaskToSend(task);
                    taskToSends.Add(temp);
                }
                Response r = new Response(taskToSends, null);
                return JsonSerializer.Serialize(r);
            }
        }
        /// <summary>		 
        ...
        {		 
            return userService.GetUserBoards(email);
        }
        /// <summary>		 
        ...
        /// <param name="emailAssignee">Email of the assignee user</param>		 
        /// <returns>An empty response, unless an error occurs (see <see cref="GradingService"/>)</returns>		 
        ...
        {		 
            return boardService.DeleteData();
        }
    }
}
